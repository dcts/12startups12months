...
- **music NFT**: you are an artist. You create an NFT, only the holder of the NFT can hear the music. Then people can "line up" to purchase the NFT to listen to the song, and then sell it to the next person. When you buy, you know for sure that there is another person after you which will buy the piece to listen to. Smart contract ensures that you can buy and sell for the same price within a given amount of time. The nft is passed along, and every time the nft is changing "hands" the creator/artist gets a small fee. The time you can hold and listen to the song is very limited. E.g. you line up and then you are assigned a specific time the NFT will be in your possesion and then you can play/listen to it. Flawed idea since when people will listen they will probably record and republish it. Or is there a way to ENSURE (with zero knowledge proof?) that sound is not recorded? Probably not. Definately not. Stupid idea. Needs improvement.
...
- **bigo.io**: flashcards for learning / evaluating BigO notations. With detailed explenations.
- **algospeedrum.io**: Algoexpert.io but for algorithms you want to be able to memorize. Use case: pick algorithms you want to "learn" or "internalize". Do them every morning and evening before going to bed in a random order. E.g. you pick => bubble sort, selection sort, reverse linked list, linked list nodeswap, powerset, getPermutations, binary tree DF and BF traversals. Then your coding environment is setup such that you can easily write those functions and you get a report after your "speed run". => how long did it take you? Whats the successrate? New record? MVP: no signup, only javascript, the chosen algorithm is encoded in url, at the end you can share your speedrun results.
...
- **QWOP Ai**: revisit this funny game and make it work, then, inspired by code bullet, do video on youtube (maybe)?
